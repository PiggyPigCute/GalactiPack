...
execute as @e[type=#itemio:container, tag=itemio.container.initialised] at @s run function itemio:v0.9.0/container/tick
execute as @e[type=hopper_minecart, tag=itemio.minecart_disabled] at @s run function itemio:v0.9.0/container/disable_minecart
execute as @e[type=#itemio:servos, tag=itemio.servo.extract, tag=itemio.servo.initialised, predicate=itemio:v0.9.0/internal/good_queue] run function itemio:v0.9.0/servo/test_already
