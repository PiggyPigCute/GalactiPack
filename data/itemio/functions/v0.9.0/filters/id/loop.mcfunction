...
data modify storage itemio:main temp.id2 set from storage itemio:io item.id
...
execute if score #!same_id itemio.math matches 1 if data storage itemio:main temp.id[0] run function itemio:v0.9.0/filters/id/loop
